...
describe(`poap-alyra-initialize with 1 mint then mint 2 - ${MAX_MINTS} then delete 1 - ${MAX_MINTS}`, () => {
  ...
  it(`Mints between 2 - ${MAX_MINTS} Nft`, async () => {
    ...
    initial_list_minted.push( newRandomNftMints )
  ...
  it(`Deletes between 1 - ${MAX_MINTS} Nft`, async () => {
    // Multiple "mints"
    const deleteNFtCount = poap_alyra_utils.getRandomPositiveInt(1, initial_list_minted.length); // between 1 and 15 mints
    const nftMintPubkeys_toDelete = [];
    // let i = 0;

    while (nftMintPubkeys_toDelete.length < deleteNFtCount) {
      // if (i > 10) break;
      const nftIdx = poap_alyra_utils.getRandomPositiveInt(0, initial_list_minted.length-1); // Get Nft between [0..items count-1]
      console.info(`nftIdx = ${nftIdx}`)
      const nftToDelete = initial_list_minted[nftIdx];
...
/*
    ...
    const totalMintsAfterDeletion = initial_list_minted.length - nftMintPubkeys_toDelete.length
    expect(userMints.totalCountMinted).to.equal(totalMintsAfterDeletion)
    expect(userMints.maxCurrentSize).to.be.lessThanOrEqual(initial_list_minted.length)
    expect(userMints.maxCurrentSize).to.be.greaterThanOrEqual(totalMintsAfterDeletion)
    expect(userMints.listMinted.length).to.equal(totalMintsAfterDeletion)

    userMints.listMinted.forEach((item: any) => {
      console.info(`remaining item = ${item}`);
    })
